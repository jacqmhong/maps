...
def group_by_centroid(restaurants, centroids):
    ...
    #     closest_centroid = find_closest(restaurant['location'], centroids)
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    # return [mean(restauraunt_location(restaurant)[0]), mean(restaurant_location(restaurant)[1])]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    for x in [(x - mean_x) ** 2 for x in xs]:
        s_xx += x
    for y in [(y - mean_y) ** 2 for y in ys]:
        s_yy += y
